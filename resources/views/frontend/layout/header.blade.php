...
<div class="container mx-auto">
    <div class="flex justify-between items-center py-4 bg-blue-900">
        <div class="flex-shrink-0 ml-10 cursor-pointer">
            ...
            <span class="ml-1 text-3xl text-blue-200 font-semibold">Bìa Hậu</span>
        </div>
        ...
        <ul class="hidden md:flex overflow-x-hidden mr-10 font-semibold">
            <li class="mr-6 p-1 border-b-2 border-orange-500">
                <a class="text-blue-200 " href="{{url('/')}}">Trang chủ</a>
            </li>
            @if(!empty($categories))
            @foreach($categories as $key => $category)
                <li class="mr-6 p-1">
                    <a class="text-white hover:text-blue-300" href="{{url('/category')}}">{{ $category->name }}</a>
                </li>
            @endforeach
            @endif
            <li class="mr-6 p-1">
                <a class="text-white hover:text-blue-300" href="{{url('/contact')}}">Liên hệ</a>
            </li>
        </ul>
    </div>
